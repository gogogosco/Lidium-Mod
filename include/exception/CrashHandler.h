#pragma once#include <../char_conversion/char_conversion.hpp>
...
static void ReadKey(HKEY hKey,
    const char * ValueName, char * Buffer, DWORD size) {
    DWORD dwType;
    ...
    if ((lErr = RegQueryValueEx(hkData, TEXT("SMBiosData"), NULL, & dwType, NULL, & dwSize)) == ERROR_SUCCESS) {
        if (dwSize == 0 || dwType != REG_BINARY)
            ...
            if (!lpData)
                ...
                lErr = RegQueryValueEx(hkData, TEXT("SMBiosData"),
    ...
    */
    ...
    do {
        ...
        if (!StackWalk(dwMachine, hProcess, hThread, & stFrame, c, NULL, SymFunctionTableAccess, SymGetModuleBase, NULL)) {
            wsprintfW(msglog, L "Last error after stack walk finished: %08X", GetLastError());
            LOG(msglog);
            break;
        }
        ...
        if (stFrame.AddrPC.Offset == 0) {
            ...
            if (!SymGetSymFromAddr(hProcess, stFrame.AddrPC.Offset, & dwOffsetFromSym, pSym2)) {
                ...
                wsprintfW(msglog, L "%s", szUndecName);
                LOG(msglog);
                if (dwOffsetFromSym)
                    wsprintfW(msglog, L " %+ld bytes", (long) dwOffsetFromSym);
                LOG(msglog);
...
LONG WINAPI OurCrashHandler(EXCEPTION_POINTERS * pExcept) {
    ...
    if ((DWORD) pExcept -> ExceptionRecord -> ExceptionAddress >= 0x70000000) {
        ...
        case EXCEPTION_ACCESS_VIOLATION:
            FaultTx = L "ACCESS VIOLATION";
            break;
        case EXCEPTION_DATATYPE_MISALIGNMENT:
            FaultTx = L "DATATYPE MISALIGNMENT";
            break;
        case EXCEPTION_FLT_DIVIDE_BY_ZERO:
            FaultTx = L "FLT DIVIDE BY ZERO";
            break;
        case EXCEPTION_ARRAY_BOUNDS_EXCEEDED:
            FaultTx = L "ARRAY BOUNDS EXCEEDED";
            break;
        case EXCEPTION_FLT_DENORMAL_OPERAND:
            FaultTx = L "FLT DENORMAL OPERAND";
            break;
        case EXCEPTION_FLT_INEXACT_RESULT:
            FaultTx = L "FLT INEXACT RESULT";
            break;
        case EXCEPTION_FLT_INVALID_OPERATION:
            FaultTx = L "FLT INVALID OPERATION";
            break;
        case EXCEPTION_FLT_OVERFLOW:
            FaultTx = L "FLT OVERFLOW";
            break;
        case EXCEPTION_FLT_STACK_CHECK:
            FaultTx = L "FLT STACK CHECK";
            break;
        case EXCEPTION_FLT_UNDERFLOW:
            FaultTx = L "FLT UNDERFLOW";
            break;
        case EXCEPTION_ILLEGAL_INSTRUCTION:
            FaultTx = L "ILLEGAL INSTRUCTION";
            break;
        case EXCEPTION_IN_PAGE_ERROR:
            FaultTx = L "IN PAGE ERROR";
            break;
        case EXCEPTION_INT_DIVIDE_BY_ZERO:
            FaultTx = L "INT DEVIDE BY ZERO";
            break;
        case EXCEPTION_INT_OVERFLOW:
            FaultTx = L "INT OVERFLOW";
            break;
        case EXCEPTION_INVALID_DISPOSITION:
            FaultTx = L "INVALID DISPOSITION";
            break;
        case EXCEPTION_NONCONTINUABLE_EXCEPTION:
            FaultTx = L "NONCONTINUABLE EXCEPTION";
            break;
        case EXCEPTION_PRIV_INSTRUCTION:
            FaultTx = L "PRIVILEGED INSTRUCTION";
            break;
        case EXCEPTION_SINGLE_STEP:
            FaultTx = L "SINGLE STEP";
            break;
        case EXCEPTION_STACK_OVERFLOW:
            FaultTx = L "STACK OVERFLOW";
            break;
        case DBG_CONTROL_C:
            FaultTx = L "CONTROL C";
            break;
